...
def center_crop2(im, new_size):
    ...
    im = im.resize((int(width / ratio), int(height / ratio)), resample=Image.LANCZOS)
...
def accuracy(preds, truth):
    ...
    top5_acc = top5_cnt / float(num_preds) * 100.0
    top1_acc = top1_cnt / float(num_preds) * 100.0
...
def server_argument_parser():
    ...
    parser.add_argument(
        "--pack_data", type=str2bool, default=True, help="Use plaintext packing on data"
    )
