...
def squash_layers(cryptonets_model, sess):
    ...
    squashed_bias = y.eval(
        session=sess, feed_dict={"input:0": np.zeros((1, 14 * 14 * 5))}
    )
    squashed_bias_plus_weights = y.eval(
        session=sess, feed_dict={"input:0": np.eye(14 * 14 * 5)}
    )
...
def save_model(cryptonets_model, sess, directory, filename):
    ...
    y_conv = model.cryptonets_model_squashed(
        x, conv1_weights, squashed_weights, fc2_weights
    )

    # squashed_model = model.cryptonets_model_squashed(x, conv1_weights, squashed_weights, fc2_weights)
    # print('squashed_model summary: ', squashed_model.summary())

    sess.run(tf.compat.v1.global_variables_initializer())

    frozen_graph = freeze_session(
        sess, output_names=["output/BiasAdd"]
    )  # out.op.name for out in squashed_model.outputs])
...
def main(FLAGS):
    ...
    cryptonets.fit(
        x_train, y_train, epochs=FLAGS.epochs, batch_size=FLAGS.batch_size, verbose=1
    )
