...
#===================================================================================================
# Provides Bash functions for dealing with yapf.
#===================================================================================================

declare _intelnervana_yapf_lib_SCRIPT_NAME="${BASH_SOURCE[${#BASH_SOURCE[@]} - 1]}"
declare _maint_SCRIPT_DIR="$( cd $(dirname "${_intelnervana_yapf_lib_SCRIPT_NAME}") && pwd )"

source "${_maint_SCRIPT_DIR}/bash_lib.sh"

yapf_lib_verify_version() {
    if (( $# != 2 )); then
        bash_lib_print_error "Usage: ${FUNCNAME[0]} <yapf-prog-pathname> <required-version-number>"
        return 1
    fi
    ...
    if ! [[ -f "${PROGNAME}" ]]; then
        bash_lib_print_error "Unable to find yapf program named '${PROGNAME}'"
        return 1
    fi
    ...
    local VERSION_X_Y_Z
    if ! VERSION_X_Y_Z=$(echo "${VERSION_LINE}" | sed ${SED_FLAGS} 's/^yapf ([0-9]+.[0-9]+.[0-9]).*$/\1/p')
    then
