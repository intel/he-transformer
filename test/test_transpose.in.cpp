...
NGRAPH_TEST(${BACKEND_NAME}, transpose) {
  //
  // Create a graph for f(x,perm) = Transpose(x,perm)
  auto x =
      std::make_shared<op::Parameter>(element::f32, PartialShape::dynamic());
  auto perm = std::make_shared<op::Parameter>(
      element::i64, PartialShape{Dimension::dynamic()});

  auto x_transpose = std::make_shared<op::Transpose>(x, perm);

  auto f = std::make_shared<Function>(NodeVector{x_transpose},
                                      ParameterVector{x, perm});
  ...
  for (size_t i = 0; i < x_shapes.size(); i++) {
    ...
    auto t_perm = backend->create_tensor(element::i64, Shape{perms[i].size()});
    ...
    ASSERT_TRUE(test::all_close_f(results, expected_results[i],
